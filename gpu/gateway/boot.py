...
# Сколько видеопамяти оставить СЕБЕ, вне vLLM: GigaAM, эмбеддер, контекст
# CUDA процесса шлюза, фрагментация.
#
# Число оценочное и намеренно с запасом. Асимметрия рисков: перебор стоит
# меньшего KV-кэша (работает, но меньше диалогов), недобор — падения загрузки
# наших моделей после того, как vLLM уже забрал свою долю. Поэтому сначала
# консервативно, а уточнять — по замеру: реальное потребление видно
# в /v1/status полем vram_ours_mb после прогрева.
#
# На 24 ГБ каждый лишний гигабайт резерва стоит ~15 тысяч токенов KV-кэша,
# то есть примерно 8 одновременных диалогов.
RESERVE_GB = float(os.getenv("RESERVE_GB", "7.0"))
