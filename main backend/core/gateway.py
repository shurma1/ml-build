...
class Gateway:
    ...
    async def embed(self, texts, kind="query"):
        ...
        Это инвариант, а не стиль: цена потерянного или продублированного
        префикса измерена и составляет -19 п.п. R@1, причём по выдаче
        это не видно — список остаётся правдоподобным.
        """
