...
# --- привязка извлечённых фактов к тексту диалога ----------------------------
#
# validate() на шлюзе проверяет ФОРМУ значения (enum, диапазоны), но не его
# опору в репликах. Здесь — вторая страховка, и она платящая: ложная
# «инвалидность» переворачивает вердикты о праве по десяткам услуг, а ложная
# ситуация дописывается в поисковый запрос и уводит retrieval. Измерено на
# живой 9B (под, extract/v1): «мне 10 лет, я хочу получить паспорт» устойчиво
# 5/5 приходит с categories=['инвалид'] и life_situation='Утрата документов',
# которых в репликах нет и быть не может.
#
# Ручная правка оператора (set_facts) через этот фильтр не идёт: для неё
# дословное свидетельство не требуется, оператор отвечает за неё сам.

# Правила в нижнем регистре и без «ё»: текст диалога нормализуется тем же
# способом. Многодетность — единственная категория, выводимая не из слов:
# трое и больше детей дают её без упоминания самого слова.
_CATEGORY_EVIDENCE = {
    "ВБД": r"вбд|ветеран\w* боев|боев\w+ действ",
    "СВО": r"\bсво\b|специальн\w+ военн\w+ операц",
    "ЧАЭС": r"чернобыл|чаэс",
    "многодетный": r"многодетн|трое детей|троих детей|трех детей|четверо детей|"
                   r"четверых детей|пятеро детей|пятерых детей|шестеро детей|"
                   r"семеро детей",
    "инвалид": r"инвалид",
    "пенсионер": r"пенсионер|на пенсии|пенсионн\w+ возраст|уш\w+ на пенсию",
    "малоимущий": r"малоимущ|малообеспечен|низк\w+ доход",
    "военнослужащий": r"военнослужащ|по контракт|мобилизованн|призванн|служу в арм",
}

# «Утрата документов» — единственная ситуация, которую глушим регуляркой:
# модель ставит её на любое упоминание документа, а ряд слов для неё узкий и
# однозначный. Остальные значения справочника — про смысл разговора, а не про
# дословное слово, и резать их наугад вреднее, чем пропускать.
_LOSS_EVIDENCE = r"потер[яюи]|утрат|укра|порва|испорт|в негодн|не годен|истек"


def grounded(facts, dialogue_text):
    """Сбрасывает категории и «Утрату документов» без опоры в репликах.

    Окно извлечения видит все реплики с начала сессии, поэтому и свидетельство
    ищется по всему тексту: факт, названный реплику назад, остаётся в силе.
    """
    out = dict(facts or {})
    low = re.sub(r"\s+", " ", (dialogue_text or "").lower().replace("ё", "е"))
    cats = out.get("categories")
    if isinstance(cats, list) and cats:
        keep = []
        for c in cats:
            rx = _CATEGORY_EVIDENCE.get(c) or re.escape(str(c).lower().replace("ё", "е"))
            inferred = (c == "многодетный" and isinstance(out.get("children"), int)
                        and out["children"] >= 3)
            if inferred or re.search(rx, low):
                keep.append(c)
        if len(keep) != len(cats):
            log.warning("категории без опоры в тексте сброшены: %s -> %s", cats, keep)
            out["categories"] = keep
    if out.get("life_situation") == "Утрата документов" and not re.search(_LOSS_EVIDENCE, low):
        log.warning("ситуация «Утрата документов» без опоры в тексте сброшена")
        out["life_situation"] = None
    return out
...
class SessionManager:
    ...
    async def _extract(self, s, seq):
        ...
        # Возраст — единственный факт, у которого есть дешёвый детерминированный
        # источник: «мне N лет» газеттир берёт без модели. Промах LLM по одному
        # полю не должен стоить вердикта о праве (паспорт в 10 лет — blocked
        # именно по возрасту), поэтому здесь страховка, а не замена извлечения.
        if facts.get("age") is None:
            from facets import find_age
            for t in turns:
                age = find_age(t.get("text") or "")
                if age is not None:
                    facts = {**facts, "age": age}
                    break

        # Форма проверена схемой на шлюзе — здесь проверяется опора в тексте:
        # что модель назовёт без свидетельства, то и сбросится (см. grounded).
        facts = grounded(facts, " ".join(t.get("text") or "" for t in turns))
