...
def test_не_спрашиваем_про_то_что_уже_прозвучало():
    ...
    знаем = _признаки(life_situation="Многодетная семья", recipient="person",
                      municipality="город Тула", facts={"categories": ["многодетный"], "age": 10})
    ключи = {q["key"] for q in suggest(cands, feats=знаем, top_n=9)}
    assert not ключи & {"life", "recipient", "municipality", "льгота", "ребенок", "age"}, ключи

    q = suggest(cands, needed=["age"], feats=_признаки(facts={"age": 10}), top_n=9)
    assert "age" not in {x["key"] for x in q}
...
# --- возраст газеттиром и честный unknown ------------------------------------

@pytest.mark.parametrize("фраза,ожидаем", [
    ("мне 10 лет", 10),
    ("мне десять лет", 10),
    ("мне двадцать три года", 23),
    ("мне уже сорок пять лет", 45),
    ("мне нужен паспорт мне 10 лет", 10),
    ("сыну 10 лет", None),                     # возраст не посетителя
    ("мне нравится этот летний день", None),
    ("по достижении 14-летнего возраста", None),   # цитата из регламента
    ("мне 130 лет", None),
])
def test_возраст_газеттиром_берётся_из_слов_и_цифр(фраза, ожидаем):
    from facets import find_age
    assert find_age(фраза) == ожидаем


def test_непроверяемые_условия_к_заявителю_не_дают_зелёный():
    """Загранпаспорт: все предикаты — по полям ('residence', 'other'), фактов
    о которых извлечение не выдаёт. Раньше это молча давало 'eligible', и услуга
    светилась зелёной даже иностранцу («я не гражданин рф»). Честный вердикт —
    unknown с дословной цитатой регламента и БЕЗ need_fact: спрашивать нечего,
    такого факта в схеме нет."""
    from eligibility import check
    t = {'type_key': 'выдача гражданина личность оформление паспортов пределами '
                     'российской территории удостоверяющих федерации',
         'recipients': []}
    статус, причины = check(t, {})
    assert статус == 'unknown'
    assert причины[0].get('unverifiable') is True
    assert 'Гражданин Российской Федерации' in (причины[0].get('quote') or '')
    assert 'need_fact' not in причины[0]


def test_документ_для_модели_включает_то_что_видит_карточка(каталог):
    """Карточка печатает «124 отделения», ведомство и правовое основание —
    модель обязана видеть их же. Иначе на «сколько отделений» она отвечает
    «в регламенте не указано», хотя оператор видит число прямо над чатом."""
    from core.routers.services import _ask_document
    doc = _ask_document("10000008589")   # загранпаспорт: 124 отделения, УМВД
    assert "124" in (doc.get("Число отделений МФЦ") or "")
    assert "по вопросам миграции" in (doc.get("Ведомство") or "")
    assert doc.get("Правовое основание")
    # карточка относит услугу к «Утрата документов» — модель обязана видеть и
    # это, иначе на «а если паспорт потеряли?» она честна, но бесполезна
    assert "Утрата документов" in (doc.get("Жизненные ситуации услуги") or "")
    # сырые текстовые поля регламента при этом не теряются
    assert doc.get("documentsText") and doc.get("rejectReasonsText")


# --- привязка извлечённых фактов к тексту ------------------------------------

def test_факты_без_опоры_в_тексте_сбрасываются():
    """Живой 9B (extract/v1) устойчиво 5/5 домудривает на «мне 10 лет, я хочу
    получить паспорт» категорию «инвалид» и ситуацию «Утрата документов».
    Привязка обязана снять и то, и другое, не трогая настоящие факты."""
    from core.sessions import grounded
    f = grounded({"age": 10, "recipient": "person",
                  "categories": ["инвалид"],
                  "life_situation": "Утрата документов"},
                 "Мне 10 лет, я хочу получить паспорт.")
    assert f["age"] == 10 and f["recipient"] == "person"
    assert f["categories"] == []
    assert f["life_situation"] is None


def test_факты_с_опорой_переживают_привязку():
    from core.sessions import grounded
    f = grounded({"categories": ["инвалид", "СВО"],
                  "life_situation": "Утрата документов"},
                 "Я инвалид, муж участник специальной военной операции, "
                 "документы утратили при переезде")
    assert f["categories"] == ["инвалид", "СВО"]
    assert f["life_situation"] == "Утрата документов"


def test_многодетность_разрешено_выводить_из_числа_детей():
    """Единственная категория с немешающимся выводом: трое детей — это и есть
    многодетность, слово в тексте не обязательно."""
    from core.sessions import grounded
    f = grounded({"categories": ["многодетный"], "children": 4},
                 "нужна выплата на ребёнка")
    assert f["categories"] == ["многодетный"]
    # ...но не из одного ребёнка
    f = grounded({"categories": ["многодетный"], "children": 1},
                 "нужна выплата на ребёнка")
    assert f["categories"] == []


def test_остальные_ситуации_привязка_не_трогает():
    """Глушить регуляркой «Выход на пенсию» и т. п. вреднее, чем пропускать:
    там модель обобщает смысл разговора, а не выдумывает слово из ниоткуда."""
    from core.sessions import grounded
    f = grounded({"life_situation": "Выход на пенсию"}, "хочу оформить выплату")
    assert f["life_situation"] == "Выход на пенсию"
